...
function DrawerOverlay({ className, ...props }: DrawerPrimitive.Backdrop.Props) {
	return <DrawerPrimitive.Backdrop data-slot="drawer-overlay" className={cn(`
		fixed inset-0 z-50 min-h-dvh bg-black/10
		opacity-[max(var(--drawer-overlay-min-opacity,0),calc(1-var(--drawer-swipe-progress)))]
		transition-opacity duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] select-none
...
function DrawerSwipeHandle({ className, ...props }: React.ComponentProps<'div'>) {
	return <div data-slot="drawer-swipe-handle" aria-hidden="true" className={cn(`
...
function DrawerContent({ children, className, ...props }: DrawerPrimitive.Popup.Props) {
	...
	return (
		<DrawerPortal data-slot="drawer-portal">
			...
			<DrawerPrimitive.Viewport data-slot="drawer-viewport" data-modal={modal} className={`
			...
			`}>
				<DrawerPrimitive.Popup
					...
					className={cn(
						...
						`
							...
							flex-col bg-popover text-sm text-popover-foreground
				...
				>
					...
					<DrawerPrimitive.Content data-slot="drawer-content" className={cn(`
