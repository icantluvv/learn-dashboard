...
function Slider({
	...
	return (
		<SliderPrimitive.Root className={cn(`
			data-horizontal:w-full
			data-vertical:h-full
		`, className)} data-slot="slider" defaultValue={defaultValue} value={value} min={min} max={max} thumbAlignment="edge" {...props}>
			...
			>
				<SliderPrimitive.Track
					...
					className="
						relative grow overflow-hidden rounded-full bg-slider-track select-none
				...
				>
					<SliderPrimitive.Indicator
						...
						className="
							bg-slider-indicator select-none
				...
				{Array.from({ length: _values.length }, (_, index) => (
					<SliderPrimitive.Thumb
						...
						className="
							relative block size-3 shrink-0 rounded-full border border-ring bg-slider-thumb
