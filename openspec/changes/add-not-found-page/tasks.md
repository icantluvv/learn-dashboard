...
## 4. Тёмная тема (последующая правка)

- [x] 4.1 `app/not-found.tsx`: `text-gray-600` → `text-muted-foreground` для описания — единственный
      нетематизированный цвет на странице; заголовок `<h1>` уже наследует `text-foreground` от
      `body`, а кнопка «На главную» уже использует тематизированный `buttonVariants({ variant:
'outline' })` из `@repo/core` (см. `dark-theme-support`). Без этой правки текст описания
      оставался тёмно-серым и терял контраст на тёмном фоне.
