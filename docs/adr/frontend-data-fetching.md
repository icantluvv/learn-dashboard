...
Next.js 16 + React 19 с App Router. Данные с бэкенда (Symfony) нужны как на сервере (SSR, стриминг), так и
на клиенте (интерактивность, фоновые обновления). OpenAPI-схемы бэкенда живут в `api/src/openapi.yaml` и
`packages/api/monolith/openapi.yaml`; из них Kubb генерирует TypeScript-типы, fetch-клиенты, Zod-схемы и React Query
хуки в `packages/api/{base,monolith}/codegen/`.
...
Все GET-запросы к Symfony API идут через TanStack Query. Прямые `fetch` в клиентских компонентах, SWR, Context-based
...
| Форма с файлами (multipart) + серверная валидация | Route Handler как BFF-прокси к Symfony                 |
