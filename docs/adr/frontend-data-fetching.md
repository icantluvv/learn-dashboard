...
Next.js 16 + React 19 с App Router. На данный момент у проекта **нет бэкенда** — реализуется только фронтенд, все
API-запросы мокаются на стороне фронтенда (MSW / моковые обработчики). Контракт API уже описан в OpenAPI-схеме
(`api/src/openapi.yaml`), из которой Kubb генерирует TypeScript-типы, fetch-клиенты, Zod-схемы и React Query хуки в
`packages/api/base/codegen/` — это позволяет писать код в предположении, что бэкенд появится позже, без переписывания
data-fetching слоя. Данные нужны как на сервере (SSR, стриминг), так и на клиенте (интерактивность, фоновые
обновления) — стратегия ниже не зависит от того, реальный это бэкенд или мок.
...
Все GET-запросы к API (сейчас — к моковым обработчикам, в будущем — к реальному бэкенду) идут через TanStack Query.
Прямые `fetch` в клиентских компонентах, SWR, Context-based кеши и ручные `useState/useEffect` для серверных данных —
не используются.
...
import { getMeProfileQueryKey, useRegisterParticipant } from '@repo/api/base'
...
| Случай                                            | Обоснование                                              |
| ------------------------------------------------- | -------------------------------------------------------- |
| Аутентификационные флоу (регистрация, logout)     | Требуют доступа к Better Auth Admin API и `Set-Cookie`   |
| Вызовы внешних partner API                        | Credentials не должны утекать в браузер                  |
| Форма с файлами (multipart) + серверная валидация | Route Handler как BFF-прокси к бэкенду (сейчас — к моку) |
...
**Причина отказа:** для приложений с формами и интерактивными состояниями клиентский кеш необходим.
