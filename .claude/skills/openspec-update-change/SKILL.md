...
description: Update an OpenSpec change by revising its existing planning artifacts and keeping them coherent with one another, including the BCP test-plan.md companion. Use when the user wants to revise a change's plan, fold new decisions into it, or reconcile its artifacts after an edit. Never edits code.
...
2. **Get the change's artifacts**
    ...
    **BCP companion file**: `<changeRoot>/test-plan.md` is not a schema artifact, so it never appears in
    `artifactPaths` or `contextFiles`. When it exists on disk, treat it as an additional editable file
    alongside the artifacts reported by the CLI. If it is missing, do not create it here - report it and
    point to `/openspec-apply-change`.
...
4. **Read and reconcile**
    ...
    - If `test-plan.md` exists, reconcile it too: a MUST/SHALL scenario added or reworded in the delta
      specs needs its row in `Scenario coverage` (risk, test level, test file, status); a removed scenario
      loses its row; changed verification steps update `## Manual checks` and `## Verification commands`.
      Revising specs while leaving an existing test-plan untouched is a coherence gap, not a neutral no-op.
