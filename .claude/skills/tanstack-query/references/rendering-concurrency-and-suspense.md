...
Исключение из `select` переводит конкретный observer в `status: "error"`
и `isSuccess: false`, но исходная data остаётся успешной в query cache;
другие observers могут продолжать её читать. `return new Error(...)` не
равнозначен `throw`: возвращённое значение рассматривается как selected data.
Проверяй HTTP status и wire data в generated query path, чтобы невалидный
ответ не попадал в общий cache. `select` оставляй для преобразования успешных data.

## Retries

| Среда                                            | Default                                                    |
| ------------------------------------------------ | ---------------------------------------------------------- |
| Query hooks в browser                            | 3 retries                                                  |
| Query hooks на server                            | 0 retries                                                  |
| Imperative `queryClient.query` / `infiniteQuery` | Без retries, если policy не задана явно или через defaults |
...
Ссылки на source ведут в текущую ветку для навигации. Проверяй поведение по
исходникам опубликованного `latest` package: `main` может содержать ещё не
...
- [`useMutation` source](https://github.com/TanStack/query/blob/main/packages/react-query/src/useMutation.ts)
- [`QueryObserver` source](https://github.com/TanStack/query/blob/main/packages/query-core/src/queryObserver.ts)
