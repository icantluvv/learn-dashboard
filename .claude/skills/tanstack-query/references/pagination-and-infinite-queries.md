...
const projectsQuery = useQuery({
	...getProjectsQueryOptions({ query: { page } }, { client: getBrowserApiClient() }),
...
```tsx
const canGoNext = !projectsQuery.isPlaceholderData && Boolean(projectsQuery.data?.hasMore)
```
...
генерирует его при `hooks: true` только для operation с настроенным
...
`initialData`, `placeholderData` и manual `setQueryData` сохраняют обе collections
и их одинаковую длину. `select` преобразует только результат observer и может,
например, вернуть плоский список; это не меняет форму cache. Если `select`
возвращает `InfiniteData`, сохраняй согласованность `pages` и `pageParams`.

Для imperative загрузки используй
`queryClient.infiniteQuery(generatedInfiniteOptions)`. Необязательный prefetch
явно обрабатывает rejection; для critical loader error должен дойти до framework.
...
- [`useInfiniteQuery` API](https://tanstack.com/query/latest/docs/framework/react/reference/useInfiniteQuery)
