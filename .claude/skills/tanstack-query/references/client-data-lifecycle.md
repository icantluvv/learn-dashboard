...
1. если package отсутствует, установи последний стабильный `@tanstack/react-query`
   текущим package manager после проверки совместимости;
...
Не передавай сырой `bigint` в key: default hash использует `JSON.stringify`.
Выбор `integerType`, нормализация identity и изменение generated keys описаны в
[openapi-kubb-runtime.md](openapi-kubb-runtime.md#числовые-типы-и-query-keys).
...
Для imperative fetch и prefetch актуальный `QueryClient` предоставляет `query`
и `infiniteQuery`. Оба возвращают data и бросают error. Для необязательного
prefetch явно обработай rejection, например `.catch(() => undefined)`; critical
loader должен передать error framework. Старые `fetchQuery`, `prefetchQuery` и
их infinite-варианты deprecated: новые примеры строй на актуальном API, а
существующие вызовы меняй только в scope задачи после проверки installed types.
...
const detailSeedOptions = {
	...
	initialDataUpdatedAt: () => queryClient.getQueryState(todoListKey)?.dataUpdatedAt,
}
```
...
бери из generated types. Mutation hook принимает options, а grouped request
`{ path, query, headers, body }` передаётся в `mutate`/`mutateAsync`:

```tsx
const updateTodo = useUpdateTodo({
	client: { client: getBrowserApiClient() },
	mutation: {
...
generated `queryFn`, `mutationFn` или transport. Изменение generated `queryKey`
проводится через config и regeneration; для runtime scope, недоступного
генератору, используй единую handwritten key/options composition по правилам
[Kubb reference](openapi-kubb-runtime.md#числовые-типы-и-query-keys).
Известный mutation response обновляй через generated type и самый узкий key family.
