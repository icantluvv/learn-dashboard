...
Freshness policy должна присутствовать у browser consumer. `dehydrate`
переносит query state, включая `dataUpdatedAt`, но не server `staleTime`.
Размещай общую policy в handwritten options рядом с generated factory:

```ts
// Общий feature module, доступный server и browser.
// GetPetByIdOptions, createClient и factory импортируются из generated modules.
type PetId = GetPetByIdOptions['path']['petId']
type ApiClient = ReturnType<typeof createClient>

export function petQueryOptions(petId: PetId, apiClient: ApiClient) {
	return {
...
Server Component получает уже разобранный route input и передаёт snapshot:

```tsx
export async function PetSection({ petId }: { petId: PetId }) {
	const apiClient = createServerApiClient()
	const queryClient = new QueryClient()

	await queryClient.query(petQueryOptions(petId, apiClient))
...
Hydrated Client Component использует тот же options builder:

```tsx
'use client'

export function Pet({ petId }: { petId: PetId }) {
	const query = useQuery(petQueryOptions(petId, getBrowserApiClient()))
	return <PetView query={query} />
}
```

`PetView` обозначает существующий view с pending/error/data states. Имена,
импорты и grouped inputs бери из generated Kubb types и структуры проекта.
Для нескольких независимых server queries используй `Promise.all`.
...
Актуальный `queryClient.query` возвращает data либо бросает error; для infinite
query используй `queryClient.infiniteQuery`. Когда route обязан вернуть
critical 404/500, передай error framework без подавления. Для необязательного
prefetch допустим явный `.catch(() => undefined)`, если предусмотрен client retry.
Default dehydration включает successful queries. Failed query передавай только
с явным `shouldDehydrateQuery`. Не переноси неявное подавление ошибок из старого
`prefetchQuery` в новый `query()` без проверки route semantics.
...
В актуальном API pending query можно включить в dehydrated state без `await`:
...
- Общий `staleTime` применён и к server preload, и к browser consumer, согласован
  с server fetch time и CDN/page cache; немедленный refetch проверен явно.
